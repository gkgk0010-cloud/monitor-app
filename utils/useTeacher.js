  ...
  const load = useCallback(async () => {
    ...
    for (let attempt = 0; attempt < 6; attempt += 1) {
      const res = await supabase
        .from('teachers')
        .select(
          'id, name, email, invite_code, academy_id, visible_menus, academy_name, academy_logo_url, teaching_type, default_test_time_per_word, default_test_question_count, default_test_pass_score, default_test_max_attempts, default_test_question_types, daily_research_quota, default_jogbo_question_count, jogbo_daily_complete_min_questions',
        )
        .eq('email', email)
        .maybeSingle();
      qErr = res.error;
